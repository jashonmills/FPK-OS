/*
JavaScript SCORM API wrapper for SCORM 1.2 & SCORM 2004
Version: 1.0.0
...
pipwerks.SCORM.API.getHandle = function() {
    ...
    if (!API.handle && !API.isFound) {
        API.handle = pipwerks.SCORM.API.get();
    }
    return API.handle;
};
        ...
        if (API) {
            ...
            if (success) {
                ...
                if (errorCode !== null && errorCode === 0) {
                    ...
                    success = false;
                    trace(traceMsgPrefix + "failed. Error code: " + errorCode);
                }
            } else {
                errorCode = debug.getCode();
                if (errorCode !== null && errorCode !== 0) {
...
pipwerks.SCORM.connection.terminate = function() {
    ...
    if (scorm.connection.isActive) {
        ...
        if (API) {
            ...
            success = (scorm.version === "1.2") ? makeBoolean(API.LMSFinish("")) : makeBoolean(API.Terminate(""));
...
pipwerks.SCORM.data.set = function(parameter, value) {
    ...
    trace(traceMsgPrefix + " value: " + value + ", success: " + success);
    return success;
};
...
pipwerks.SCORM.init = function() {
    return pipwerks.SCORM.connection.initialize();
};
...
pipwerks.SCORM.set = function(parameter, value) {
    return pipwerks.SCORM.data.set(parameter, value);
};

pipwerks.SCORM.save = function() {
    return pipwerks.SCORM.data.save();
};
...
pipwerks.SCORM.getStatus = function() {
    return pipwerks.SCORM.status("get");
};

pipwerks.SCORM.setStatus = function(status) {
    return pipwerks.SCORM.status("set", status);
};
