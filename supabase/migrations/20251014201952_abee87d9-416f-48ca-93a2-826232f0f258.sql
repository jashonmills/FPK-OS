-- Create the get_student_knowledge_pack function
-- This function gathers all critical learning context for a student
...
DECLARE
  ...
  v_org_context JSONB;
BEGIN
  -- 1. ACTIVE COURSES (Current enrollments with progress)
  SELECT jsonb_agg(
    jsonb_build_object(
  ...
  -- 2. RECENT ACTIVITY (Last 7 days of learning activity)
  SELECT jsonb_build_object(
    'recent_lessons', (
      SELECT jsonb_agg(
        jsonb_build_object(
          'lesson_title', lc.lesson_title,
          'course_id', lc.course_id,
          'completed_at', lc.completed_at,
          'time_spent_seconds', lc.time_spent_seconds
  ...
  -- 3. ACTIVE GOALS (Current learning objectives)
  SELECT jsonb_agg(
    jsonb_build_object(
      'title', g.title,
      'category', g.category,
  ...
  -- 4. STRENGTHS & WEAKNESSES (Performance patterns)
  SELECT jsonb_build_object(
    'strengths', (
  ...
  -- 5. INSTRUCTOR NOTES (For org students)
  SELECT jsonb_agg(
    jsonb_build_object(
      'note', sn.note_text,
  ...
  -- 6. LEARNING PATTERNS (Study habits & preferences)
  SELECT jsonb_build_object(
    'current_streak', get_coach_streak(p_user_id),
    'preferred_study_times', (
      SELECT jsonb_agg(DISTINCT EXTRACT(HOUR FROM created_at)::int ORDER BY EXTRACT(HOUR FROM created_at)::int)
  ...
  -- 7. GAMIFICATION STATUS (XP, badges, achievements)
  SELECT jsonb_build_object(
  ...
  -- 8. RECENT STRUGGLES (Areas needing attention)
  SELECT jsonb_agg(
    jsonb_build_object(
  ...
  -- 9. AI COACH INSIGHTS (Phoenix system intelligence)
  SELECT jsonb_build_object(
  ...
  -- 10. ORGANIZATION CONTEXT (For org students)
  SELECT jsonb_build_object(
  ...
  -- Assemble the complete knowledge pack
