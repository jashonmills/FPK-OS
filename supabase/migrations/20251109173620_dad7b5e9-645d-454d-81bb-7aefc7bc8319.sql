-- ================================================================
-- EMERGENCY FIX: get_student_knowledge_pack() Schema Corrections
-- ================================================================
-- Fixes:
-- 1. time_spent_seconds → total_time_spent_minutes (line 25)
-- 2. mastery_score → (completion_percentage / 100.0) (lines 115, 123, 130)
    ...
    'active_courses', COALESCE(
      (SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'course_slug', c.slug,
          'enrollment_status', ice.enrollment_status,
      ...
      WHERE ice.user_id = p_user_id
        AND ice.enrollment_status IN ('active', 'in_progress')
      ), '[]'::jsonb
    ),
