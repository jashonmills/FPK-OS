...
const TARGET_URLS = [
  {
    url: "https://education.ucsb.edu/autism/pivotal-response-treatment",
    title: "Pivotal Response Treatment (PRT) Overview"
  }
];
  ...
  try {
    ...
    for (const target of TARGET_URLS) {
      ...
      const { data: kbEntry, error: kbError } = await supabase
        .from("knowledge_base")
        .insert({
          source_name: "Koegel Autism Center",
          document_type: "therapeutic_model",
          title: target.title,
          source_url: target.url,
          focus_areas: ["autism", "prt", "evidence-based-interventions"],
          keywords: ["PRT", "Pivotal Response Treatment", "autism intervention", "naturalistic teaching"]
        })
        .select()
        .single();
    ...
    return new Response(
      JSON.stringify({
        message: "PRT Resources scraping completed",
    ...
    console.error("Error in scrape-prt-resources:", error);
    return new Response(
