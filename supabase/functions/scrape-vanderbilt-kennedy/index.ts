...
const TARGET_URLS = [
  {
    url: "https://vkc.vumc.org/vkc/research-development",
    title: "Vanderbilt Kennedy Center Research"
  },
  {
    url: "https://vkc.vumc.org/vkc/resources",
    title: "Vanderbilt Kennedy Center Resources"
  }
];
  ...
  try {
    ...
    for (const target of TARGET_URLS) {
      ...
      const { data: kbEntry, error: kbError } = await supabase
        .from("knowledge_base")
        .insert({
          source_name: "Vanderbilt Kennedy Center",
          document_type: "practice_brief",
          title: target.title,
          source_url: target.url,
          focus_areas: ["special-education", "evidence-based-interventions"],
          keywords: ["research to practice", "developmental disabilities", "special education"]
        })
        .select()
        .single();
    ...
    return new Response(
      JSON.stringify({
        message: "Vanderbilt Kennedy scraping completed",
    ...
    console.error("Error in scrape-vanderbilt-kennedy:", error);
    return new Response(
