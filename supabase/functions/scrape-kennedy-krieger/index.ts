...
const TARGET_URLS = [
  {
    url: "https://www.kennedykrieger.org/patient-care/patient-care-programs/autism-related-disorders",
    title: "Autism Related Disorders Programs"
  },
  {
    url: "https://www.kennedykrieger.org/patient-care/patient-care-programs/center-for-autism-and-related-disorders",
    title: "Center for Autism and Related Disorders"
  }
];
  ...
  try {
    ...
    for (const target of TARGET_URLS) {
      ...
      const { data: kbEntry, error: kbError } = await supabase
        .from("knowledge_base")
        .insert({
          source_name: "Kennedy Krieger Institute",
          document_type: "clinical_resource",
          title: target.title,
          source_url: target.url,
          focus_areas: ["autism", "clinical-care"],
          keywords: ["autism", "clinical resources", "family support"]
        })
        .select()
        .single();
    ...
    return new Response(
      JSON.stringify({
        message: "Kennedy Krieger scraping completed",
    ...
    console.error("Error in scrape-kennedy-krieger:", error);
    return new Response(
