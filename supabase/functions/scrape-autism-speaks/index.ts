...
const TARGET_URLS = [
  {
    url: "https://www.autismspeaks.org/tool-kit/100-day-kit",
    title: "Autism Speaks 100 Day Kit"
  },
  {
    url: "https://www.autismspeaks.org/tool-kit/transition-tool-kit",
    title: "Autism Speaks Transition Tool Kit"
  }
];
...
serve(async (req) => {
  ...
  try {
    ...
    for (const target of TARGET_URLS) {
      ...
      const { data: kbEntry, error: kbError } = await supabase
        ...
        .insert({
          ...
          focus_areas: ["autism", "family-support"],
          keywords: ["autism toolkit", "family resources", "parent guide"]
        })
        .select()
        .single();
    ...
    return new Response(
      JSON.stringify({
        message: "Autism Speaks scraping completed",
    ...
    console.error("Error in scrape-autism-speaks:", error);
    return new Response(
