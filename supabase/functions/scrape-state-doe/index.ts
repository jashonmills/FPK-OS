...
const STATE_URLS: Record<string, { url: string; title: string }[]> = {
  "CA": [
    {
      url: "https://www.cde.ca.gov/sp/se/qa/psnrghtsnt.asp",
      title: "California Special Education Parent Rights"
    }
  ],
  "TX": [
    {
      url: "https://tea.texas.gov/academics/special-student-populations/special-education",
      title: "Texas Special Education Resources"
    }
  ],
  "NY": [
    {
      url: "https://www.nysed.gov/special-education/parent-guide-special-education",
      title: "New York Parent Guide to Special Education"
    }
  ]
};
  ...
  try {
    ...
    for (const target of targetUrls) {
      ...
      const { data: kbEntry, error: kbError } = await supabase
        .from("knowledge_base")
        .insert({
          source_name: `${state} Department of Education`,
          document_type: "state_guidance",
          title: target.title,
          source_url: target.url,
          focus_areas: ["special-education", "legal", state.toLowerCase()],
          keywords: ["state guidance", "special education", "parent rights", state]
        })
        .select()
        .single();
    ...
    return new Response(
      JSON.stringify({
        message: `${state} State DOE scraping completed`,
        state,
    ...
    console.error("Error in scrape-state-doe:", error);
    return new Response(
