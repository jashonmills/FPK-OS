...
const TARGET_URLS = [
  {
    ...
    title: "CHADD ADHD Overview"
  },
  {
    url: "https://chadd.org/for-parents/overview/",
    title: "CHADD Resources for Parents"
  }
];
...
serve(async (req) => {
  ...
  try {
    ...
    for (const target of TARGET_URLS) {
      ...
      const { data: kbEntry, error: kbError } = await supabase
        ...
        .insert({
          ...
          focus_areas: ["adhd"],
          keywords: ["ADHD", "attention deficit", "parent resources"]
        })
        .select()
        .single();
    ...
    return new Response(
      JSON.stringify({
        message: "CHADD scraping completed",
    ...
    console.error("Error in scrape-chadd:", error);
    return new Response(
