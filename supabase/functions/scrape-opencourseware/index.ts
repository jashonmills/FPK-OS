...
const TARGET_URLS = [
  {
    ...
    title: "MIT OCW - Introduction to Psychology"
  },
  {
    url: "https://ocw.mit.edu/courses/9-85-cognition-in-infancy-and-early-childhood-fall-2012/",
    title: "MIT OCW - Cognition in Infancy and Early Childhood"
  }
];
...
serve(async (req) => {
  ...
  try {
    ...
    for (const target of TARGET_URLS) {
      ...
      const { data: kbEntry, error: kbError } = await supabase
        ...
        .insert({
          ...
          document_type: "academic_course",
          title: target.title,
          source_url: target.url,
          focus_areas: ["academic", "psychology", "education"],
          keywords: ["OpenCourseWare", "MIT", "psychology", "developmental psychology"]
        })
        .select()
        .single();
    ...
    return new Response(
      JSON.stringify({
        message: "OpenCourseWare scraping completed",
    ...
    console.error("Error in scrape-opencourseware:", error);
    return new Response(
