...
serve(async (req) => {
  ...
  try {
    ...
    if (isResumingThisTurn) {
      ...
      console.log('[CONDUCTOR] 🔄 Resetting Nite Owl trigger and setting resumption lock');
      ...
      lastNiteOwlTurn = conversationHistory.length; // Lock Nite Owl from triggering on this turn
    }
...
IMPORTANT: Only use "request_for_clarification" when the student explicitly asks for a definition or states they don't understand a specific term. Regular questions should still be "socratic_guidance" or "direct_answer".`
    ...
    if (isResumingThisTurn) {
      ...
      if (selectedPersona === 'BETTY') {
        systemPrompt = buildBettySystemPrompt(userMemories, knowledgePack) + `\n\n---\n\nCRITICAL INSTRUCTION: The student is RETURNING to this conversation after a break. Your task is to:

1. Welcome them back warmly (e.g., "Welcome back, ${user?.user_metadata?.name || 'there'}!")
2. Briefly summarize where you left off in the learning conversation
3. Ask if they're ready to continue or if they'd like to review anything first

Recent conversation context:
${recentContext}

DO NOT simply repeat your last question. Generate a NEW, contextual greeting that acknowledges their return.`;
      } else {
        systemPrompt = buildAlSystemPrompt(studentContext, userMemories, knowledgePack) + `\n\n---\n\nCRITICAL INSTRUCTION: The student is RETURNING to this conversation after a break. Welcome them back and briefly summarize what you were discussing before.
