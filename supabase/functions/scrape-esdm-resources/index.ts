...
const TARGET_URLS = [
  {
    url: "https://health.ucdavis.edu/mindinstitute/research/esdm/",
    title: "Early Start Denver Model (ESDM) Research"
  }
];
...
serve(async (req) => {
  ...
  try {
    ...
    for (const target of TARGET_URLS) {
      ...
      const { data: kbEntry, error: kbError } = await supabase
        ...
        .insert({
          ...
          focus_areas: ["autism", "early-intervention", "esdm"],
          keywords: ["ESDM", "Early Start Denver Model", "early intervention", "autism therapy"]
        })
        .select()
        .single();
    ...
    return new Response(
      JSON.stringify({
        message: "ESDM Resources scraping completed",
    ...
    console.error("Error in scrape-esdm-resources:", error);
    return new Response(
