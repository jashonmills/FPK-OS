...
const TARGET_URLS = [
  {
    url: "https://health.ucdavis.edu/mindinstitute/research/autismresearch.html",
    title: "MIND Institute Autism Research Overview"
  },
  {
    url: "https://health.ucdavis.edu/mindinstitute/about/index.html",
    title: "About the MIND Institute"
  }
];
  ...
  try {
    ...
    for (const target of TARGET_URLS) {
      ...
      const { data: kbEntry, error: kbError } = await supabase
        .from("knowledge_base")
        .insert({
          source_name: "UC Davis MIND Institute",
          document_type: "research_summary",
          title: target.title,
          source_url: target.url,
          focus_areas: ["autism", "research"],
          keywords: ["autism research", "neurodevelopment", "MIND Institute"]
        })
        .select()
        .single();
    ...
    return new Response(
      JSON.stringify({
        message: "UC Davis MIND Institute scraping completed",
    ...
    console.error("Error in scrape-ucdavis-mind:", error);
    return new Response(
