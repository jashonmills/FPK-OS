...
BEGIN
  ...
  -- Calculate average progress across students
  SELECT COALESCE(AVG(
    CASE 
      WHEN (progress->>'completion_percentage')::numeric IS NOT NULL 
      THEN (progress->>'completion_percentage')::numeric 
      ELSE 0 
    END
  ), 0)
  ...
  SELECT CASE 
    WHEN COUNT(*) > 0 THEN (
      COUNT(*) FILTER (WHERE (progress->>'completion_percentage')::numeric >= 100)::numeric 
      / COUNT(*)::numeric * 100
    )
    ELSE 0
  END
...
BEGIN
  ...
  AND (e.progress->>'completion_percentage')::numeric >= 100;
  ...
  FROM (
    SELECT 
      e.user_id,
      AVG((e.progress->>'completion_percentage')::numeric) as avg_progress,
    ...
    GROUP BY e.user_id
    HAVING AVG((e.progress->>'completion_percentage')::numeric) > 0
    ORDER BY avg_progress DESC
    LIMIT 5
  ) top;
...
BEGIN
  ...
  FROM enrollments
  WHERE user_id = p_student_id
    AND (progress->>'completion_percentage')::numeric >= 100;
  ...
  -- Average progress
  SELECT COALESCE(AVG((progress->>'completion_percentage')::numeric), 0)
  INTO v_average_progress
  FROM enrollments
  WHERE user_id = p_student_id;
