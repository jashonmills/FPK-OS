-- Fix ambiguous created_at column reference in get_student_knowledge_pack
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'visual', COALESCE((preferences->>'visual')::int, 0),
    'auditory', COALESCE((preferences->>'auditory')::int, 0),
    'kinesthetic', COALESCE((preferences->>'kinesthetic')::int, 0),
    'reading_writing', COALESCE((preferences->>'reading_writing')::int, 0)
  )
  INTO v_learning_style
  FROM profiles
  WHERE id = p_user_id;

  -- 2. PREFERRED SUBJECTS (from user activity)
  ...
  -- 8. RECENT STRUGGLES - FIX: Explicitly qualify created_at column
  SELECT jsonb_agg(
    jsonb_build_object(
  ...
  -- 9. KNOWLEDGE GAPS (topics with low success rates)
  SELECT jsonb_agg(
    jsonb_build_object(
      'topic', topic,
  ...
  -- Return comprehensive knowledge pack
